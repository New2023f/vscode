...
if (isOSS) {
	...
	productJsonPath = path.join(root, 'distro', 'mixin', process.env.VSCODE_QUALITY!, 'product.json');
}

console.error('Loading product.json from', productJsonPath);
const product = readJSON(productJsonPath);
const allProductsAndQualities = isOSS ? [product] : fs.readdirSync(path.join(root, 'distro', 'mixin'))
	.map(quality => ({ quality, json: readJSON(path.join(root, 'distro', 'mixin', quality, 'product.json')) }));
